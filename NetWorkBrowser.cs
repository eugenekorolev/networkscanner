...
namespace ListNetworkComputers
{
    ...
    /// This collection of PC names is used in the <see cref="ListNetworkComputers.mainWindow">
    /// form </see>
    /// <br></br>
    ...
    public sealed class NetworkBrowser
    {
        ...
        //create a _SERVER_INFO_101 STRUCTURE
        [StructLayout(LayoutKind.Sequential)]
        ...
        {
            //local fields
            ArrayList networkComputers100 = new ArrayList();
            ArrayList networkComputers101 = new ArrayList();
            const int MAX_PREFERRED_LENGTH = -1;
            int SV_TYPE_WORKSTATION = 1;
            int SV_TYPE_SERVER = 2;
            IntPtr buffer = IntPtr.Zero;
            IntPtr tmpBuffer100 = IntPtr.Zero;
            IntPtr tmpBuffer101 = IntPtr.Zero;
            int entriesRead = 0;
            int totalEntries = 0;
            int resHandle = 0;
            int sizeofINFO100 = Marshal.SizeOf(typeof(_SERVER_INFO_100));
            int sizeofINFO101 = Marshal.SizeOf(typeof(_SERVER_INFO_101));
     ...
     //                   tmpBuffer100 = new IntPtr((int)buffer + (i * sizeofINFO100));
     ...
     //                       Marshal.PtrToStructure(tmpBuffer100, typeof(_SERVER_INFO_100));

     //                   //add the PC names to the ArrayList
     //                   networkComputers100.Add(svrInfo);


     //                   //tmpBuffer101 = new IntPtr((int)buffer101 + (i * sizeofINFO101));
     //                   //_SERVER_INFO_101 svrInfo1 = (_SERVER_INFO_101)
     //                   //    Marshal.PtrToStructure(tmpBuffer101, typeof(_SERVER_INFO_101));
     //                   //networkComputers101.Add(svrInfo1);
     //               }
     //           }
                
                int ret = NetServerEnum(null, 101, ref buffer, MAX_PREFERRED_LENGTH,
                    out entriesRead,
                    out totalEntries, SV_TYPE_WORKSTATION | SV_TYPE_SERVER, null, out
                    resHandle);
                ...
                if (ret == 0)
                {
                    ...
                    for (int i = 0; i < totalEntries; i++)
                    {
                        tmpBuffer101 = new IntPtr((int)buffer + (i * sizeofINFO101));
                        _SERVER_INFO_101 svrInfo = (_SERVER_INFO_101)
                            Marshal.PtrToStructure(tmpBuffer101, typeof(_SERVER_INFO_101));
                        networkComputers101.Add(svrInfo.sv101_name);
                    }
                }
            }
            catch (Exception ex)
